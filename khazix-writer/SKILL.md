...
**排比句的呼吸控制**：多个结构相同的排比句，用分号「；」连接，形成一口气推到底的势能；句号则用来制造断裂和停顿。两种节奏各司其职，分号让读者跟着情绪往上冲，句号让读者停下来感受重量。典型场景：文章结尾的情感堆叠段落，三个以上的「是我们……」用分号串成一句长呼吸。

❌ 「是我们明知道……勇气。是我们明知道……倔强。是我们明知道……角落。」
✅ 「是我们明知道……勇气；是我们明知道……倔强；是我们明知道……角落。」
...
8. **「不是…而是…」矫正句式** ⚠️ Isaac 铁律：禁止用否定一个解释来确立另一个解释。包括"不是X，是Y""既不是…也不是…""并不是…而是…"等全部变体。受访者原话引用可以豁免。详见 L1-5 自动扫描。
...
⚠️ **必须运行全部五项。L1-5（矫正句式）是最高频漏检项——肉眼检出率极低，必须用 grep/Python 自动扫描。**

**推荐使用 `scripts/l1_scan.py`** 一键运行全部五项检查：
```bash
python3 ~/.hermes/skills/khazix-writer/scripts/l1_scan.py <article.md>
```
或者自行写脚本，但必须覆盖 L1-1 到 L1-5 全部五项，缺一不可。
...
**L1-5 矫正句式扫描** ⚠️ 最高频漏检项
全文搜索以下模式，出现则必须改写：
- `不是…而是…` / `不是…是…` — "不是X，是Y"矫正句式
- `既不是…也不是…` — 双重否定矫正
- `并不是…而是…` / `不只是…而是…` — 变体

⚠️ 禁止「用否定一个解释来确立另一个解释」的句子结构。这是 AI 最爱的修辞手段，也是 Isaac 明确禁止的写作模式。

**检查命令**: `grep -n '不是\|既不是'`
**注意**: 受访者直接引用（「」内的原话）可以豁免。单纯否定（"它不是人"）、疑问句（"是不是越容易买单"）、"不只是X，也是Y"类的范围扩展不算矫正句式。

**替代方案**:
- ❌ "不是因为它喜欢你，是因为它要你订阅" → ✅ "跟喜不喜欢没关系。是它背后的公司需要你订阅"
- ❌ "它研究的不是AI，是人的脆弱" → ✅ "把AI拿掉之后，剩下的东西，是人的脆弱"
- ❌ "不是AI脆弱。是我们脆弱。" → ✅ "AI好着呢。我们反倒是脆弱的那一方。"
- 通用技巧：「反倒是」「从来跟……无关」「跟……没关系」这三个词组可以直接替代绝大多数「不是X，是Y」结构，不需要每次都重新造句。

**通过标准**：以上五项扫描零命中。
**修复方式**：逐个替换，用正向陈述或过程性描述替代矫正句式。
