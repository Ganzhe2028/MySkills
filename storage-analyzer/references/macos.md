...
**⚠️ APFS `du` 陷阱：** 对 Container 子目录（`Data/Desktop/`、`Data/Documents/` 等）逐项 `du` 时，各项之和可能远超父目录——APFS 的 clone/hard link 使同一份数据被多次计入。分析时应以 `scan.py` 的顶层 `du` 结果为准，子目录 `du` 仅用于摸清内容结构、不作为实际大小依据。常见例子：微信 `Data/` 报 22 GB，但 `Data/Desktop/` + `Data/Documents/` 各自报 27 GB 和 22 GB（总和远超 22）。
