...
把分析结果写成 analysis JSON。**必须平铺在顶层**（`green`/`yellow`/`red` 直接是 DATA 的键，不能套 `tiers`），字段名严格匹配模板期望：

- 🟢 项：`name` `path` `size` `type` `detail` `trash_paths`（必填！） `close_apps` `commands`（可选）
- 🟡 项：`name` `path` `size` `type` `detail` `content_profile` `why_manual` `disposal` `risk` `trash_paths`（可选） `open_note`（可选）
- 🔴 项：`name` `path` `size` `type` `detail` `why_keep` `indirect_release` `app_paths`（可选）

`schema` 参考：`scripts/build_report.py` 顶部注释。
...
**排障：网页上没有删除/移废纸篓按钮（连批量按钮也不见）** = 三项排查：
1. 是否开了静态报告？→ 改用 `server.py`
2. 🟢 项是否漏了 `trash_paths`？→ 补上重启服务
3. **JSON 结构是否把 green/yellow/red 嵌套在了 `tiers` 键下？** → 模板读的是 `DATA.green` / `DATA.yellow` / `DATA.red`，必须平铺在顶层，不能套一层 `tiers`（`build_report.py` 的 schema 注释说明的就是平铺结构）。修完 JSON 后必须 kill 旧 server 重启才能生效。

**排障：server.py 后台模式 stdout 被缓冲** → `process action='log'` 输出为空，看不到端口号和 token。两个办法：
1. 加 `-u` 去缓冲：`python3 -u scripts/server.py /tmp/storage_analysis.json`
2. 用 `lsof -p <pid> -i TCP -s TCP:LISTEN` 找到端口，token 对本地页面来说不需要（server.py 的 `webbrowser.open(url)` 会自动带上 token）。

**排障：`webbrowser.open(url)` 在 Hermes 后台进程里可能没效果** → 手动执行 `open http://127.0.0.1:<port>/` 打开浏览器。
